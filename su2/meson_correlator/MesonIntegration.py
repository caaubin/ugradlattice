...
def calculate_meson_mass(U, mass: float, channel: str, lattice_dims: List[int], 
    ...
    if channel.lower() == 'pion':
        return calculate_pion_mass(U, mass, lattice_dims, wilson_r=wilson_r, solver=solver, verbose=verbose)

    elif channel.lower() == 'sigma':
        return calculate_sigma_mass(U, mass, lattice_dims, wilson_r=wilson_r, solver=solver, verbose=verbose)

    elif channel.lower() == 'rho':
        return calculate_rho_mass(U, mass, lattice_dims, 'average', wilson_r=wilson_r, solver=solver, verbose=verbose)

    elif channel.lower() in ['rho_x', 'rho_y', 'rho_z']:
        polarization = channel.split('_')[1]  # Extract x, y, z
        return calculate_rho_mass(U, mass, lattice_dims, polarization, wilson_r=wilson_r, solver=solver, verbose=verbose)
...
def calculate_meson_spectrum(U, mass: float, lattice_dims: List[int], 
    ...
    try:
        pion_result = calculate_pion_mass(U, mass, lattice_dims, wilson_r=wilson_r, solver=solver, verbose=verbose)
    ...
    if include_rho_polarizations:
        ...
        try:
            rho_result = calculate_rho_mass(U, mass, lattice_dims, 'average', wilson_r=wilson_r, solver=solver, verbose=verbose)
    ...
    try:
        sigma_result = calculate_sigma_mass(U, mass, lattice_dims, wilson_r=wilson_r, solver=solver, verbose=verbose)
